        ...
        for (const VisualRun &visualRun : lineSpan.visualRuns) {
            ...
            for (int j = static_cast<int>(visualRun.start); j < static_cast<int>(visualRun.end); ++j) {
                ...
                const Vector2f offset {
                    static_cast<float>(caret.x - std::floor(caret.x)),
                    static_cast<float>(caret.y - std::floor(caret.y)),
                };
                                   ...
                                   ShapeResult& result)
{
    ...
    if (!faceItem) {
        ...
        if (inserted) {
            log_.warn("Paragraph shaping error: Missing face item '{}'", seq.format.faceId);
        }
        return;
    }
    ...
    for (unsigned i = 0; i < hbLen; ++i) {
        ...
        if (isSkipGlyph(paragraph.text_[i])) {
            log_.info("Skipping unrecognized glyph '{}'", paragraph.text_[i]);
            continue;
        }
